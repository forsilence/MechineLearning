...
typedef struct cluster
{
  ...
  cluster(data *initial_pointer){
   centroid = new std::vector<double>;
   cluster_points = new std::vector<data *>;
   for(auto value : *(initial_pointer->get_feature_vector())){
     centroid->push_back(value);
   }
   cluster_points->push_back(initial_pointer);
   class_counts[initial_pointer->get_label()] = 1;
   most_frequent_class = initial_pointer->get_label();
  }
