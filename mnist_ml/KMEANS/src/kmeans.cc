...
double kmeans::validate()
{
  ...
  return 100.0 * (num_corrent / (double)validation_data->size());
}
double kmeans::test()
{
  double num_corrent = 0.0;
  ...
  return 100.0 * (num_corrent / (double)test_data->size());
}
...
int main()
{
  ...
  for (int k = dh->get_class_counts(); k <dh->get_training_data()->size() * 0.1; ++k){
    ...
    printf("current Preformace @ K = %d: %.2f\n", k, performance);
