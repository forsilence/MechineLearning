...
void knn::find_knearest(data *query_point)
{
  ...
  for (int i = 0; i < k; ++i){
    if (i == 0){
      ...
      for (int j = 0; k < training_data->size(); ++j){
...
double knn::calculate_distance(data *query_point, data* input)
{
  ...
  // printf("%f \n",distance);
#elif defined(MANHATIAN)
#endif
  return distance;
}
double knn::validate_preformance()
{
  double current_preformance = 0;
  int count = 0;
  int data_index = 0;
  ...
  printf("Validation Performance for K = %d: %.3f %%\n", k, current_preformance);
  return current_preformance;
}
...
int main()
{
  ...
  for (int i = 1; i <= 4; ++i){
    if (i == 1) {
      knearest->set_k(i);
      performance = knearest->validate_preformance();
